...
这是最简单的协作方式，适合小团队（如课程小组）。仓库所有者邀请组员成为 Collaborator，所有人都可以直接推送代码。

**设置步骤：**

1. 仓库所有者：Settings → Collaborators → Add people → 输入组员的 GitHub 用户名
2. 组员：接受邮件中的邀请
...
所有人都直接推送到同一个分支，容易产生冲突。务必遵守：

- **每次开始工作前先 `git pull`**，拉取队友的最新修改
- 团队成员尽量编辑不同的文件
- 频繁提交、频繁推送，减少冲突范围
:::
...
当你想给别人的项目贡献代码，但没有直接推送权限时，使用 Fork + PR 流程：

1. **Fork**：在 GitHub 上将别人的仓库复制一份到自己的账号
2. **Clone**：将 Fork 的仓库克隆到本地
3. **修改**：在本地创建新分支，进行修改并提交
4. **Push**：推送到自己的 GitHub 仓库
5. **Pull Request（PR）**：向原仓库发起合并请求，由原仓库维护者审核

```bash
# 1. Fork 后克隆自己的仓库（替换为你的 GitHub 用户名）
git clone https://github.com/your-username/repo-name.git

# 2. 创建新分支进行修改（不要直接在 main 上改）
...
# 5. 在 GitHub 网页上点击 "Compare & pull request" 创建 PR
```
...
| 场景 | 推荐模式 |
|:---|:---|
| 课程小组项目 | Collaborator 直接协作 |
| 给开源项目贡献代码 | Fork + Pull Request |
| 公司/实验室内部项目 | Collaborator + 分支保护 |
:::
